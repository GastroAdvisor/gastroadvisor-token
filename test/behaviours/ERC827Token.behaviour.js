...
export default function (accounts) {
  ...
  describe('validating allowance updates to spender', function () {
    ...
    it('should start with zero', async function () {
      preApproved = await this.token.allowance(accounts[0], accounts[1]);
      assert.equal(preApproved, 0);
    });

    it('should increase by 50 then decrease by 10', async function () {
      await this.token.increaseApproval(accounts[1], 50);
      let postIncrease = await this.token.allowance(accounts[0], accounts[1]);
      preApproved.plus(50).should.be.bignumber.equal(postIncrease);
      await this.token.decreaseApproval(accounts[1], 10);
      let postDecrease = await this.token.allowance(accounts[0], accounts[1]);
  ...
  it('should increase by 50 then set to 0 when decreasing by more than 50', async function () {
    await this.token.approve(accounts[1], 50);
    await this.token.decreaseApproval(accounts[1], 60);
    let postDecrease = await this.token.allowance(accounts[0], accounts[1]);
  ...
  describe('Test ERC827 methods', function () {
    ...
    it(
      ...
      , async function () {
        ...
        const transaction =
          await this.token.transferFromAndCall(accounts[0], message.contract.address, 100, extraData, {
            from: accounts[1],
          });
